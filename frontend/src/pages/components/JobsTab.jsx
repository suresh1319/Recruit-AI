...
import JobCandidatesDashboard from '../JobCandidatesDashboard';
...
export default function JobsTab() {
    ...
    const [viewingJobIdForCandidates, setViewingJobIdForCandidates] = useState(null);
    ...
    const handleMatchCandidates = async (jobId) => {
        ...
        try {
            ...
            if (response.ok) {
                const data = await response.json();
                const newMatches = data.matches?.filter(m => m.matchScore > 75).length || 0;
                const rejected = data.rejectedCount || 0;
                if (newMatches > 0 || rejected > 0) {
                    toast.success(`${newMatches} matched, ${rejected} auto-rejected (score < 75).`);
                } else {
                    toast.info('No high-score candidates found for this role.');
                }
                fetchJobs();
            } else {
        ...
        <div key={candidate._id} className="p-4 border border-slate-200 dark:border-slate-800 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
            <div className="flex justify-between items-start mb-2">
                ...
                <div className="flex flex-col items-end gap-2 shrink-0">
                    ...
                    {['Screening Pending', 'pending', 'matched', 'invited', 'called', 'scheduled'].includes(candidate.status) && (
                        <div className="flex flex-col gap-2 mt-2">
                            <Button
                                size="sm"
                                disabled={sendingInviteId === candidate._id}
                                className="bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg gap-1.5 h-8 w-full disabled:opacity-60 disabled:cursor-not-allowed"
                                onClick={async () => {
                                    setSendingInviteId(candidate._id);
                                    ...
                                    try {
                                        ...
                                        if (response.ok) {
                                            toast.success('Invite sent! Email is on its way.', { id: toastId });
                                            fetchJobs();
                                        } else {
                                        ...
                                        toast.error('Network error. Could not send invite.', { id: toastId });
                            ...
                            >
                                {sendingInviteId === candidate._id
                                    ...
                                    : <><Mail size={14} /> {candidate.interviewLink || ['invited', 'called', 'scheduled'].includes(candidate.status) ? 'Invite Again' : 'Send Invite'}</>
                                }
                            </Button>
                            <Button
                                size="sm"
                            ...
                            {candidate.interview?.status === 'Completed' && candidate.interview?.interviewId && (
                                <Button
                                    size="sm"
                                    variant="outline"
                                    className="border-indigo-200 dark:border-indigo-800 text-indigo-600 dark:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-950/30 rounded-lg gap-1.5 h-8 w-full font-bold transition-colors"
                                    onClick={() => {
                                        setShowCandidatesModal(false);
                                        navigate(`/report/${candidate.interview.interviewId}`);
                                    }}
                                >
            ...
            {candidate.interview && (
                <div className="flex flex-wrap items-center gap-2 mb-2">
                    <span className={`px-2.5 py-0.5 text-[10px] font-bold uppercase rounded-full flex items-center gap-1 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300`}>
                    ...
                    {candidate.interview.status === 'Completed' && candidate.interview.recommendation && (
                        <span className={`px-2.5 py-0.5 text-[10px] font-bold uppercase rounded-full ${
                            candidate.interview.recommendation === 'Recommended' ? 'bg-emerald-100 dark:bg-emerald-950/30 text-emerald-700 dark:text-emerald-400' :
                            candidate.interview.recommendation === 'Not Recommended' ? 'bg-rose-100 dark:bg-rose-950/30 text-rose-700 dark:text-rose-400' :
                            'bg-indigo-100 dark:bg-indigo-950/30 text-indigo-750 dark:text-indigo-400'
                        }`}>
    ...
    if (viewingJobIdForCandidates) {
        return (
            <JobCandidatesDashboard
                jobId={viewingJobIdForCandidates}
                onBack={() => {
                    setViewingJobIdForCandidates(null);
                    fetchJobs();
                }}
            />
        );
